...
int main()
{
   ...
   while(n--)
   {
      ...
      cin >> date;
      //cout << date;
      int monthi = stoi(date.substr(0,2));
      int dayi = stoi(date.substr(2,2));
      int yeari = stoi(date.substr(4,4));
      Date born = Date(yeari, monthi, dayi);
      born.addDay(280);

      cout << born << " ";

      string sign[13] = {"capricorn", "aquarius", "pisces", "aries", "taurus", "gemini", "cancer",
			 "leo", "virgo", "libra", "scorpio", "sagittarius", "capricorn"};

      Date begin[13] = {Date(born.yyyy, 01, 01), Date(born.yyyy, 01, 21), Date(born.yyyy, 02, 20), Date(born.yyyy, 03, 21),
			Date(born.yyyy, 04, 21), Date(born.yyyy, 05, 22), Date(born.yyyy, 06, 22),
      Date(born.yyyy, 07, 23), Date(born.yyyy, 8, 22), Date(born.yyyy, 9, 24),
			Date(born.yyyy, 10, 24), Date(born.yyyy, 11, 23), Date(born.yyyy, 12, 23)};

      for(int i = 0; i < 13; i++)
      {
        if(born >= begin[i] && born < begin[i+1])
        {
          cout << sign[i] << endl;
        }
      }
